...
class BankBuilder {
	...
	static aEuropeanBank = () =>
		BankBuilder.aBank().withPivotCurrency(Currency.EUR);
	...
	build() {
		const bank = Bank.withExchangeRate(
			this.pivotCurrency,
			this.exchangeRates[0].currency,
			this.exchangeRates[0].rate
		);
		this.exchangeRates.forEach((exchangeRates) => {
			bank.addExchangeRate(
				this.pivotCurrency,
...
describe("BankBuilder", () => {
	test("builds a bank with a pivot currency", () => {
		const myBank = BankBuilder.aEuropeanBank()
			.withExchangeRate(Currency.USD, 1.2)
			.build();
		expect(myBank.convert(Currency.USD, new Money(10, Currency.EUR))).toBe(
			12
		);
	});
	...
	test("convert from eur to usd returns number", () => {
		...
		expect(
			myBank.convert(Currency.USD, new Money(10, Currency.EUR))["amount"]
		).toBe(12);
	});
	...
	test("convert from current money to the same money returns same value", () => {
		...
		expect(
			myBank.convert(Currency.EUR, new Money(10, Currency.EUR))["amount"]
		).toBe(10);
	});
	...
	test("convert with different exchange rates returns different numbers", () => {
		const myBank = BankBuilder.aEuropeanBank()
			.withExchangeRate(Currency.USD, 1.2)
			.build();
		const myBank2 = BankBuilder.aEuropeanBank()
			.withExchangeRate(Currency.USD, 1.3)
			.build();
		const myBank3 = BankBuilder.aEuropeanBank()
		...
		expect(
			myBank2.convert(Currency.USD, new Money(10, Currency.EUR))["amount"]
		).toBe(13);
		expect(
			myBank3.convert(Currency.USD, new Money(10, Currency.EUR))["amount"]
		).toBe(15);
	});
});
